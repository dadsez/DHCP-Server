...
public class DHCPSocket extends DatagramSocket
{
  ...
  /**
   * Sends a MensajeDHCP object to a predifined host.
   *
   * @param inMessage Well-formed MensajeDHCP to be sent to a server
   *
   * @throws IOException If the message could not be sent.
   */
  public synchronized void send( MensajeDHCP inMessage ) throws IOException
  {
    byte data[] = new byte[mtu];
    data = inMessage.DCHPaBytes();
  ...
  /**
   * Receives a datagram packet containing a DHCP Message into a MensajeDHCP
   * object.
   *
   * @return <code>true</code> if message is received, <code>false</code> if
   * timeout occurs.
   *
   * @param outMessage MensajeDHCP object to receive new message into
   */
  public synchronized boolean receive( MensajeDHCP outMessage )
  {
    try
    {
      DatagramPacket incoming = new DatagramPacket( new byte[mtu], mtu );

      receive( incoming );

      outMessage.BytesADHCP( incoming.getData() );
    }
